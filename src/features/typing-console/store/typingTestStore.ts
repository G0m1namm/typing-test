...
const useTypingStoreBase = create<TypingStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    setTypeLogs: (newLog: LogData) =>
      set((state) => ({ typeLogs: [...state.typeLogs, newLog] })),
    startTest: () =>
      set({ startTime: Date.now(), endTime: null, status: "RUNNING" }),
    endTest: (initialText: string) => {
      const endTime = Date.now();
      const { startTime, typeLogs } = get();

      const initialTime = startTime as number;
      const calculatedWPM = calcWordsPerMinute(
        initialText.length,
        endTime - initialTime,
      );
      set({ wpm: calculatedWPM });

      const { finalAccuracy, deletedErrorCount } =
        calcAccuracyAndDeletions(typeLogs);
